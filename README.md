...
- Casting-animation negative checks and optional compatibility coverage (normal visuals user-confirmed)
...
User reports all normal animations look good. Reuse this visual confirmation and previous gameplay evidence; optional Prominence and multiplayer visibility remain unverified. User reported no visible activation for both Agony and Torment with no target; exact retry cooldowns were not measured. Consecration: user observed approximately 58 seconds after restart; source and local config are 60 seconds. Exact initial cooldown/haste was not captured, so the two-second difference is not precisely verified. User confirmed the added Anoint Weapon particles are absent after restart. Registry audit: 73 effects (71 unconditional, two requiring Paladins). Of these, 65 map to the completed core/class/Ascendancy trees; reuse their recorded main-scope checks, without treating this static mapping as new per-effect runtime evidence. Eight are optional Prominence effects: Focus, Titans Grip and six Melodies; their compatibility mechanics remain unverified locally. Golden Aegis below-cap gain is user-confirmed: three zombie melee blocks gave three stacks after clearing the effect. Final-tier shield briefly visible at normal tick rate; size comparison deferred by agreement, with tier selection, damage and consumption already verified. At user request, Agony now uses the stated 8-second base: 160 + Ascendancy points ticks (8 seconds + 0.05 seconds per point; 9.5 seconds at 30 points). EN/FR/RU tooltips say 8 seconds. JSON parsing and Gradle build passed. After reporting the update loaded, user queried Agony on a Warden and observed 165 ticks remaining (8.25 seconds), consistent with the 190-tick duration at 30 points after command-entry time. Exact initial duration was not captured; tooltip display remains unverified. User elected to move on. First-use/rejoin regression: user reports Anoint Weapon worked normally after rejoining in the requested first-press check; the intermittent issue did not reproduce. User also confirmed normal Anoint Weapon expiration and recasting. This is a basic buff lifecycle observation, not verification of every expiration callback. Stop generic repeat checks of completed tree mechanics; further runtime checks must identify a concrete unresolved behavior and expected result. Dedicated development server startup passed: IDE Server configuration with working directory run/dedicated-test, fresh dedicated-test-world, loopback 127.0.0.1:25565. Log reached Done and Puffish loaded the Simply Skills data pack. Client connection passed: user entered the world and the dedicated server logged Dev joining. Anoint Weapon activation, animation, buff and cooldown are user-confirmed over the dedicated-server connection. Arcane Bolt server-side damage is verified by target health 20 -> 16.6 after the user cast; two-player visibility remains pending; this is not a packaged-JAR server test. Local server files/configs are ignored by Git. Initial Client login failed with online authentication. Local dedicated-test server now uses online-mode=false for the unauthenticated IntelliJ Dev player, with server-ip=127.0.0.1 retained. User stopped the server cleanly; restart reached Done and loopback port 25565 is listening. Offline mode is only for this local development setup. User confirmed skill trees and icons display on the dedicated-server client. Server log confirms Cleric and Anoint Weapon unlock commands succeeded; user confirmed animation, Anointed buff and cooldown on casting. Local RCON is enabled with a generated password stored only in ignored server.properties; listener verified on 127.0.0.1:25575. Ignored run/dedicated-test/Send-LocalCommand.ps1 handles commands (launch with powershell -NoProfile -ExecutionPolicy Bypass -File); list returned zero players after restart. RCON setup/helper/password do not transfer through Git. Assistant can now send server commands without asking the user to type them. Dev rejoined; assistant prepared a tagged stationary husk (server_arcane_test), confirmed 20 health and player Arcane Spell Power 1.0. Initial Wizard switch left Cleric category unlocked, conflicting with the normal one-specialisation guard. Assistant then locked Cleric category and retried Wizard category/Arcane Bolt unlock through RCON. User then cast Arcane Bolt; RCON read the tagged husk health at 16.6, down from 20 (3.4 damage), matching the 3.4 coefficient at Arcane Spell Power 1.0. Dedicated-server projectile damage is verified for this cast. User confirmed seeing Arcane Bolt fly toward the husk. Basic dedicated-development-server checks passed at this scope: login, skill-tree display, buff activation/animation/cooldown, projectile visibility and server-side damage. Second-player visibility remains unverified. Next release-readiness work: test the packaged JAR outside the IDE and remove temporary unlock-testing support before release; server remains running locally. When switching test classes with default server restrictions, lock the previous category as well as its active node; a command success message alone does not prove the category bypassed the mod guard. Command argument order checked against installed Puffish Skills classes. Detailed Ascendancy limitations above, multiplayer and release cleanup remain.
...
After animations, continue direct registered-effect checks using shared-effect evidence, check the corrected Agony duration tooltip after reload, and complete regression/performance review. Remove temporary `removeUnlockRestrictions` support before release. Keep pending checks explicit and push completed changes with this handoff.
## Main porting work

...
Packaged-JAR verification is underway outside IntelliJ: the user installed the built JAR in their modpack, entered a world, and confirmed the skill menus/trees and icons looked correct. Current fix: user reported the projectile model flying sideways during the Arcane Bolt packaged test. Changed all four Arcane Bolt variants from ALONG_MOTION to TOWARDS_MOTION, following the previously verified Arcane Slash/Rapidfire correction. JSON parsing, Gradle build and IntelliJ build passed. Replace the modpack JAR after fully closing Minecraft and visually retest Arcane Bolt; orientation remains unverified after this edit. Then continue the post-removal Ascendancy/item paths. These are user observations; external modpack logs were not inspected. Dedicated-server packaged startup and removal of temporary testing support remain release work. Existing gameplay evidence still counts.

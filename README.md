...
- Active next test (user-requested): pause Righteous Hammers testing and recheck Undying after the shared expiration-hook change. Probes are ready for lethal-hit protection, health at expiration, and the pending-death result. First scenario: in Survival, without regeneration or other protective effects, directly apply `simplyskills:undying` for 8 seconds, take lethal generic damage during that window, and remain below 60% maximum health through expiration. Expect survival while active and death after expiration. A separate recovered-health scenario follows. The direct effect test isolates the shared lifecycle; Bone Armor's grant/duration was already verified. Hammers probes were removed and must be restored when returning to that ability.
...
- Testing scope: reuse shared-effect verification from earlier trees; focus Ascendancy checks on triggers, scaling, upgrades, and interactions. Next ability is Righteous Hammers at 30 spent points: verify 5 hammers/800 ticks and the 30-point ranged hammer branch against a hostile target outside the 4-block orbit area but within 10 blocks. Probes are prepared for the cast result and the first ranged spell-call attempt. Switch only the Bone Armor node (`1ftcmq4xhgmz4zmd`) to Righteous Hammers (`xpfympjreeee2xg3`) to preserve the spent-point total. No Righteous Hammers Ascendancy results have been captured yet.

...
- Debugger state: per-tick flight probe removed. Three release-only RighteousShieldEffect probes now capture actual stack count and selection of basic/five-stack projectile. Current test: five-stack selection passed. Added tagged-zombie damage probe for upgraded damage and six-target chain (initial hit plus five ricochets); only five target hits were recorded; add one nearby spare target, protect the group from sunlight with helmets, then repeat a five-stack cast and inspect target availability. Probe expressions read effect amplifier directly, so stack counts stay correct after the source change. Higher tiers, blocking and cap remain pending. Old Magic Circle probe remains user-owned.

...
- Debugger state: completed Ghostwalk probes removed. Rapidfire probes prepared for duration, held weapon, arrow count and Marksmanship. Bow and modded crossbow duration/Marksmanship cadence verified. Keep probes until remaining checks are resolved. Added a Warden-only incoming-damage logpoint at SimplySkills.onLivingIncomingDamage (first statement), capturing entity ID, health before each hit, incoming amount and source. Compare successive health values to distinguish actual loss from incoming damage; no Warden results yet. Old Magic Circle probe remains user-owned.

Current test: Rapidfire (bow and modded crossbow traces recorded; visual/hit confirmation pending). Selection commands for resuming:

...
- Debugger state: Client running with the consumption fix. PlayerEntityMixin lines 63/65 record melee-block Aegis; HelperMethods line 293 logs Golden Aegis current/new/cap with caller stack. Repeated reported blocks produced no traces. Direct paused inspection confirmed Dev has 30 points, Righteous Shield unlocked, offhand minecraft:shield and Aegis XVIII; latest attacker Skeleton and damage source arrow. Installed NeoForge LivingEntity source calls blockUsingShield only for non-projectile damage with a living direct attacker, explaining why arrows do not reach this Aegis hook. Arrow-block gain is unsupported by current implementation; no behavior change made. Existing XVIII does not independently verify block gain or sustained cap. Debugger resumed. Remaining block test needs a melee attacker; do not ask for more arrow-block repetitions. Old Magic Circle probe remains user-owned.

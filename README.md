...
- **Magic Circle checks complete:** visible circle, immobilization/recovery, healing gesture, 241 ticks at 1 point and 270 ticks at 30 points, and +30% power in all six schools passed. Corrected the Echo bonus to require owning Wizard Spell Echo. Post-fix runtime checks confirmed 15% with the passive locked inside the circle, and 15% outside / 25% inside with it unlocked. Builds passed; probes removed. Current ability: Arcane Slash; see the local checkpoint below.
...
- Current local setup: Arcane Slash with **30 spent Ascendancy points**, confirmed by debugger. Upgraded projectile branch fires at 15 remaining ticks of the 20-tick effect. Arcane Attunement rose from 12 to 16 stacks, then capped at 19, refreshing to 60 ticks; this exposed stale amplifier-style arguments after the helper changed to stack counts. Arcane Slash now grants floor(points/10) stacks per use (3 at 30), matching its tooltip, with a hard 20-stack cap corresponding to the original nominal amplifier limit 19. The original helper could overshoot that threshold; the corrected helper clamps it. Gradle build passed; Post-restart trace confirmed 3 then 6 stacks at 30 points, each lasting 60 ticks; roll 80 correctly stopped the chain. The seeded 18-stack test then reached exactly 20 stacks and stayed at 20 through four automatic repeats, refreshing duration to 60 ticks; roll 91 ended the chain. No breakpoint errors. Stack progression and cap are verified. Recorded repeat rolls below 80 recast and roll 92 stopped the chain; no probe errors. Earlier 1-point test confirmed no Attunement, and user confirmed repaired base-slash visuals after restart. Both projectile variants use TOWARDS_MOTION to remove the extra 90-degree rotation. User tentatively reported a larger projectile. Three-target row test recorded sequential player-sourced hits on zombie IDs 2351/2353/2355 at three-block spacing. Each initially received 2.106432 incoming damage; subsequent health was 17.893568, confirming that amount was applied. Later volleys increased to 2.225664, 2.3448958 and higher while Attunement accumulated. An earlier row was interfered with by a Dreadglare; use the later row for this result. Basic piercing and applied damage are observed; exact coefficient/power correlation and the 10-target boundary remain unverified. Next: prepare an isolated boundary test; no further Attunement retest needed. Home-only configs must not be assumed here. Signature and Ascendancy use separate keys.
...
- Debugger state: ArcaneSlashEffect probes at lines 33, 41, and 54 capture initial points/duration/attunement, the upgraded projectile branch, and repeat rolls. No probe errors in the initial run. An old Magic Circle probe remains in this IDE, reported as user-owned. SimplySkills.java effect hooks were inspected; no change was made to them for this report.

Arcane Slash selection commands for an existing Magic Circle setup (already selected on this computer):

...
- Natural-expiration check is pending a repeat: the user completed a no-damage wait after a confirmed 30-point/seven-layer cast, but the removal probe still filtered for amplifier 0 and could not observe expiration at amplifier 6. No layer-decrement events were recorded. The probe in `BoneArmorEffect.onEffectRemovedCustom` now captures every server-player removal and logs the amplifier plus Undying/Regeneration, so the repeat can directly establish whether the follow-up effects are absent.

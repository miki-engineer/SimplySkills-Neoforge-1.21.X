...
- Righteous Shield first trace at 30 points verified passive Golden Aegis gain at player ticks 1200 and 1600, successful casts with one stack, base projectile branch (amplifier 0 at duration 10), and complete consumption of that stack. Cast attempts without Aegis also captured. User clarified that the shield hits an enemy but does not ricochet. Installed Spell Engine inspection shows a default ricochet_range of 5 and a nearby eligible-target search; actual target availability is unverified. Next reproduction uses three stationary zombies spaced 2 blocks apart (tag shield_test) and a low-stack throw. Controlled throw produced one tagged-zombie damage event (ID 1049, health before 20, incoming damage 0.96278244), with base-tier release and stack consumption confirmed; no further tagged hits captured. Root cause remains unresolved. Added shield-only flight probe at RangerAbilities.signatureRangerElementalArtillery first statement, which is called from the shared projectile tick: captures position, velocity, ricochet/bounce counts, search range, followed target and nearby zombies. Next: repeat one low-stack throw at the same group; no source fix applied yet. Base projectile config has bounce 5, ricochet 2, range 32 and velocity 3.6. Other tiers, blocking trigger and cap remain pending.
- Debugger state: completed Rapidfire/Warden probes removed. Righteous Shield probes prepared for Golden Aegis gains, cast stacks, release tier and remaining effects. Current check: low-stack ricochet against tagged stationary zombies. Effect-state probe restricted to the server release tick to avoid duplicate client/per-tick output. Block-trigger generation, stack cap and other projectile tiers remain pending. Old Magic Circle probe remains user-owned.

...
Buff-particle preview added to 15 activations: Berserker three signatures (rage); Rogue three, Elemental Arrows, Elemental Surge/Spellweaver and Magic Circle (arcane); Anoint Weapon, Consecration, Righteous Hammers and Chainbreaker (holy); Bone Armor (skull). Each server activation sends only 12 rising sparks using Spell Engine particle types; the outward ring was removed at user request. No repeating aura or gameplay change. Sacred Onslaught hand gesture removed. Installed particle API inspected; IntelliJ compilation and Gradle build passed. Pending after restart: preview Anoint Weapon and Consecration, confirm rising motion, visibility and acceptable density, then sample rage/arcane/skull themes. First buff test disconnected with EncoderException on vanilla level_particles: the codec rejected the spawned ParticleGroupType instance. Fixed the shared helper to send a ParticleGroup through Spell Engine ParticleHelper.sendBatches, retaining 12 upward PIPE particles and no outward ring. Installed sender API inspected; IntelliJ compilation and Gradle build passed. Pending full restart: confirm Anoint Weapon remains connected and particles rise correctly, then sample other themes. All new particle visuals and multiplayer visibility remain unverified.
...
After one full Client restart, test the new gestures in third person by toggling nodes. Agony/Torment and Rogue gestures are confirmed. Ranger Elemental Arrows and Arrow Rain are user-confirmed. Spellblade gestures are user-confirmed. Next: Cleric Anoint Weapon, then Crusader Sacred Onslaught and Consecration, one signature at a time. Disengage gesture removal loads on the next restart. Berserker final-shout visual confirmation remains pending unless explicitly confirmed; then continue the remaining class groups above. Confirm a suitable gesture, normal pose recovery, and no playback errors; watch interactions with movement and effect-driven follow-up animations. Failed Agony/Torment casts should not gesture. Gameplay regression and multiplayer visibility remain pending; static coverage is not a visual pass.

...
- **Magic Circle checks complete:** visible circle, immobilization/recovery, healing gesture, 241 ticks at 1 point and 270 ticks at 30 points, and +30% power in all six schools passed. Corrected the Echo bonus to require owning Wizard Spell Echo. Post-fix runtime checks confirmed 15% with the passive locked inside the circle, and 15% outside / 25% inside with it unlocked. Builds passed; probes removed. Arcane Slash checks are also complete at the scope below; next is Torment.
...
- Agony main checks complete at the accepted scope; currently selected with 30 spent points. Application duration verified at 201 ticks for 1 point and 230 ticks for 30 points. Four paired trigger traces confirmed 0.7175 bonus damage applied and 0.1025 health restored per hit (player health 12.933332 to 13.343332). No breakpoint errors. Main bonus-damage and 30-point healing mechanics verified for the same-player caster scenario; multiplayer caster ownership is unverified. User accepted the original healing amount and duration scaling; preserve 10% of Healing spell power per trigger and +1 tick per point. No balance change requested. Duration tooltip still says 8s while the original implementation uses a 200-tick baseline; wording correction remains pending. User requested shorter tests first. Planned order: Torment, Chainbreaker, then Ghostwalk, followed by the more involved Rapidfire, Righteous Shield, Skyward Sunder and Cataclysm checks; adjust if failures add work. Torment main 30-point checks are now complete: curse and Taunt applied for 190 ticks (9.5 seconds); six redirected 3.0-damage hits reduced the zombie health (20 to 17.06 on the first, eventually 0), while player health remained 8.975332 through the sequence. No breakpoint errors. Applied damage is subject to target mitigation. Reuse prior shared Taunt coverage; lower-point comparison and post-expiration hit behavior were not separately checked here. Current test: Chainbreaker selected at 30 points. Before/after trace showed Slowness II becoming Slowness I rather than being cleansed; Weakness was absent before the cast, so its removal was not tested. Might IV, Marksmanship IV and Undying applied for 120 ticks. Fixed Chainbreaker locally to remove complete non-beneficial effects from a snapshot instead of decrementing them via buffSteal. Shared helper unchanged. Gradle build passed; in-game fix verification pending. Restart and verify multiple negative effects of level II or higher are removed while beneficial effects remain; reuse earlier Undying survival coverage.
...
- Debugger state: Torment probes removed. Chainbreaker before/after probes capture points, health and effects; rebind them to the first statement and final return after the cleanse edit. Old Magic Circle probe remains user-owned.

Chainbreaker selection commands (currently selected):

```mcfunction
/puffish_skills category unlock @s simplyskills:ascendancy
/puffish_skills skills lock @s simplyskills:ascendancy ntkotv5t9pxmo20r
/puffish_skills skills unlock @s simplyskills:ascendancy c97wxgqn3utj7y01
```

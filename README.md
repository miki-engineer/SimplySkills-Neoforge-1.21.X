...
- Righteous Shield at 30 points: passive Aegis generation verified every 400 player ticks; low-stack casts release the base projectile and consume Aegis. Ricochet failure traced on projectile 1374: after hitting zombie 1049, ricochet count fell 2 to 1 and target 1016 was selected, but the same tick moved the shield past that target (next x=-204.284, target x=-205.40) without another collision check. Subsequent ticks continued away with one ricochet remaining. Added a targeted SpellProjectileMixin return hook for successful base/second-tier Righteous Shield ricochets: set Spell Engine skipTravel so the redirected path is collision-checked on the next tick. Velocity/damage unchanged. After restart, two throws hit all three tagged zombies for 0.96278244 incoming damage each; repeated health readings confirmed damage. First ricochet now hits directly, but final target was only hit after a wall bounce. Flight showed the return path crossing previously hit enemies; engine collision selection includes them even though onHitEntity rejects their impactHistory IDs. Added a shield-only canHitEntity filter for already-hit IDs so they cannot mask an unhit target in the same ray. Gradle build passed. After the second fix, three throws directly hit zombies 32, 33 and 34 in order, with ricochet counts 2 to 1 to 0 and no wall bounce. Incoming damage was 0.96278244 each; successive casts confirmed matching health loss. Basic-tier ricochet fix verified. Base projectile settings: bounce 5, ricochet 2, range 32, velocity 3.6. Exact five-stack test captured stacks=5 selecting the basic projectile. Fixed tier comparisons to use getAmplifier()+1 and thresholds >=5, >=10 and >=15; previously all upgrades required an extra stack. Gradle build passed; after restart, exact five-stack trace selected righteous_shield_projectile_2 with no probe errors. Upgraded throw recorded five distinct tagged targets (20, 21, 329, 327, 22), each receiving 1.8350061 incoming damage. Follow-up listed six nearby tagged zombies and hit all six (20,21,559,329,327,22) for 1.8524826 incoming damage each, confirming five ricochets. Passive gain raised stacks to 6 for that throw, within the same tier; exact five-stack selection had already passed. Ten- and fifteen-stack boundaries remain pending. Higher-tier behavior, block-trigger Aegis and stack cap remain pending.
...
- Debugger state: completed damage/tier probes removed. Two RighteousShieldEffect probes prepared: healing-tier release (line 40) records stack count and nearby shield_heal_test wolf health; final effect tick (line 54) records wolf health and remaining Aegis. User tamed a wolf manually and saw impact particles; no healing trace captured because the probes require shield_heal_test tagging. Next: tag the nearest tamed test wolf, set its health to 5 for the healing check, seed exactly 10 Aegis stacks and cast through it. Particles alone do not verify restored health. Added an unconditional cast-entry probe in AscendancyAbilities to show Aegis and nearby wolves if setup is still missing. Ten-stack selection/healing, fifteen-stack behavior, blocking and cap remain pending. Old Magic Circle probe remains user-owned.

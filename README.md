...
- Startup performance: user reported world-entry lag. Current IDE log records a 4136 ms server backlog after joining and 2132 ms during the first shield cast. First flight probe events had a 6.5-second gap; later casts completed promptly. Debugger overhead is a possible contributor, not a proven cause. All agent probes removed; compare world entry without them, then a normal non-debug launch if needed. No performance code change made.
- Debugger state: all agent probes removed after the basic shield ricochet verification, including the per-projectile flight probe. Old Magic Circle probe remains user-owned. Recreate focused probes for higher-tier/block/cap checks as needed.

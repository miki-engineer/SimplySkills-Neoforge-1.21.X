...
- Rapidfire bow trace verified at 30 spent points: 150-tick duration (7.5 seconds), 23 projectile-dispatch calls, and one Marksmanship stack per third dispatch, reaching VII. No breakpoint errors. Projectile impacts/damage and visual animation/movement/action recovery are not yet explicitly confirmed. Next: repeat with a crossbow, then resolve remaining checks. Arrow counter ended at 2; cross-cast reset and multiplayer counter isolation are unverified.
...
- Debugger state: completed Ghostwalk probes removed. Rapidfire probes prepared for duration, held weapon, arrow count and Marksmanship. Bow duration and Marksmanship cadence verified. Keep probes for crossbow and remaining checks. Old Magic Circle probe remains user-owned.

Current test: Rapidfire (bow trace recorded; crossbow next). Selection commands for resuming:

...
- Debugger state: blocking probes removed. Client running; SkywardSunderEffect line 38 logs duration, spent points, position and velocity at 45/30/15/1 ticks. Next: switch from Righteous Shield to Skyward Sunder and first verify dash, upward launch, downward slam, animations and movement recovery on open ground. Damage/scaling and upgrades remain pending. Arrow blocks do not invoke the current melee-only Aegis hook; no behavior change made. Old Magic Circle probe remains user-owned.

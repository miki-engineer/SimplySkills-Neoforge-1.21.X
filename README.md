...
Resumed on the home computer: GitHub updates pulled cleanly; local IntelliJ compilation and Java 21 Gradle build passed. Skyward Sunder movement passed at 30 points: duration 45 ticks, forward dash, approximately seven-block rise, and return to ground by the final tick. User confirmed animations and normal movement recovery. Damage at 30 points verified for the upward strike and final slam: each used multiplier 1.8 and raw damage 11.2319996; target 28 health changed 73.3664 -> 62.134403 -> 50.902405. No charge-phase hits or midair hits were captured in this cast. Charge reached target 28 twice at exactly 30 points but Death Mark was null both times: the source incorrectly requires >30 instead of >=30. Boundary corrected to >=30; live regression pending. Strike/slam also confirmed at raw 10.8 each (health 49.902405 -> 39.102406 -> 28.302406). Might-to-Barrier regression failed: Might III with 1015 ticks and no prior Barrier produced Barrier II for 1015 ticks. The call passes the zero-based Might amplifier to a helper expecting real stack counts. Corrected conversion to amplifier + 1, retaining the existing nine-stack cap. Both fixes passed IntelliJ compilation and the Java 21 Gradle build; next retest Might III -> Barrier III and charge Death Mark at exactly 30 points. Eleven of thirteen Ascendancy abilities have completed their main checks at the scopes below. Resume with Skyward Sunder, then Cataclysm; Skyward Sunder movement is now verified, while Cataclysm remains untested. Latest code passed IntelliJ compilation and the Java 21 Gradle build, and Righteous Shield consumption fixes passed after a full client restart. Final animation/effect audits and regression work remain after these two abilities.
...
- Debugger handoff: all agent probes removed for the stop; user-owned probes retained. Recreate a non-suspending SkywardSunderEffect line 38 probe for duration/points/position/velocity at 45/30/15/1 ticks when resuming. First verify dash, upward launch, downward slam, animations and movement recovery on open ground; then damage/scaling and upgrades. Skill-switch commands were supplied but execution was not confirmed. Cataclysm skill ID: avw95rphcs85w4j9. Arrow blocks do not invoke the current melee-only Aegis hook; no behavior change made.

Current test: Skyward Sunder. Switch from Righteous Shield:

```mcfunction
/puffish_skills category unlock @s simplyskills:ascendancy
/puffish_skills skills lock @s simplyskills:ascendancy jltm2vtk7wm4krdp
/puffish_skills skills unlock @s simplyskills:ascendancy m9xj5d6nb1bfpou8
```

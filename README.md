...
Next: investigate the remaining curse/taunt target-state concern in [release-review.md](docs/release-review.md). Packaged-JAR verification also remains pending: install `build/libs/simplyskills-1.7.2+1.21.1-neoforge.jar` in a separate Minecraft 1.21.1 NeoForge instance with required dependencies, then verify startup, world loading, skill menu, one class cast and one Ascendancy cast. Existing gameplay evidence still counts.

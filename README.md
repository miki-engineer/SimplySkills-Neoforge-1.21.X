...
Resumed on the home computer: GitHub updates pulled cleanly; local IntelliJ compilation and Java 21 Gradle build passed. Skyward Sunder movement probes are prepared, but the first local cast is pending. Eleven of thirteen Ascendancy abilities have completed their main checks at the scopes below. Resume with Skyward Sunder, then Cataclysm; neither has a verified runtime test in this handoff. Latest code passed IntelliJ compilation and the Java 21 Gradle build, and Righteous Shield consumption fixes passed after a full client restart. Final animation/effect audits and regression work remain after these two abilities.

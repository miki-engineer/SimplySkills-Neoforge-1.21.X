...
- Current local setup: Arcane Slash with **30 spent Ascendancy points**, confirmed by debugger. Upgraded projectile branch fires at 15 remaining ticks of the 20-tick effect. Arcane Attunement rose from 12 to 16 stacks, then capped at 19, refreshing to 60 ticks; this matches the current helper call, but intended stack count/cap still needs review against the original implementation. Recorded repeat rolls below 80 recast and roll 92 stopped the chain; no probe errors. Earlier 1-point test confirmed no Attunement, and user confirmed repaired base-slash visuals after restart. Both projectile variants use TOWARDS_MOTION to remove the extra 90-degree rotation. Next: confirm upgraded visual size, review Attunement scaling/cap, and verify damage/piercing. Home-only configs must not be assumed here. Signature and Ascendancy use separate keys.

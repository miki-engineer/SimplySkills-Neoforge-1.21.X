...
User reports all normal animations look good. Reuse this visual confirmation and previous gameplay evidence; optional Prominence, failed Agony/Torment casts and multiplayer visibility remain unverified. Consecration: user observed approximately 58 seconds after restart; source and local config are 60 seconds. Exact initial cooldown/haste was not captured, so the two-second difference is not precisely verified. Next quick regression: check Agony/Torment with no valid nearby enemy. Continue the registered-effect coverage audit without repeating shared mechanics already verified. Buff particle removal still needs a restart check.

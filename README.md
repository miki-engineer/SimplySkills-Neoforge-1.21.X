...
- Debugger state: completed Ghostwalk probes removed. Rapidfire probes prepared for duration, held weapon, arrow count and Marksmanship. Bow and modded crossbow duration/Marksmanship cadence verified. Keep probes until remaining checks are resolved. Added a Warden-only incoming-damage logpoint at SimplySkills.onLivingIncomingDamage (first statement), capturing entity ID, health before each hit, incoming amount and source. Successive Warden health values confirmed first-hit damage; final health was not captured. Old Magic Circle probe remains user-owned.

Current test: Rapidfire (bow and modded crossbow traces recorded; damage verified; visual/movement confirmation pending). Selection commands for resuming:

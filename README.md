...
- Debugger state: flight and damage probes removed. Three release-only RighteousShieldEffect probes now capture actual stack count and selection of basic/five-stack projectile. Current test: clear Golden Aegis, give amplifier 4 (five stacks), then immediately cast at the existing zombie group; verify the exact five-stack boundary before testing larger ricochet groups. Higher tiers, blocking and cap remain pending. Old Magic Circle probe remains user-owned.

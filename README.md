...
- Debugger state: flight and damage probes removed. Three release-only RighteousShieldEffect probes now capture actual stack count and selection of basic/five-stack projectile. Current test: restart with the tier-count fix, clear Golden Aegis, give amplifier 4 (five stacks), then immediately cast. Probe expressions read effect amplifier directly, so stack counts stay correct after the source change. Higher tiers, blocking and cap remain pending. Old Magic Circle probe remains user-owned.

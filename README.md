...
- **Remaining animation work:** audit active abilities across the other classes and assign suitable Spell Engine gestures where absent. Magic Circle now uses Spell Engine's one-handed healing release gesture; its animation asset exists and IntelliJ compilation plus the Java 21 Gradle build passed, but visual confirmation is pending the next client restart. Bone Armor and Righteous Hammers still lack casting gestures. Earlier class gameplay completion does not mean every animation is verified. Current fixes play gestures alongside immediate effects; no charge-time/delayed-effect system was added.

...
- Input reproduction clarified: the user sees a roughly one-second cooldown on pressing the ability immediately after entering the world. No fresh input trace has yet captured this case. Probes now capture the client key decision, the server-selected ability/success result, and every cooldown reply (including type `none`). Next reproduction: leave to title, re-enter the same world, press Ascendancy once immediately, and wait five seconds without pressing again. This is an investigation, not a confirmed synchronization bug.
- Current investigation: the user reported needing many Ascendancy keypresses before activation after the restart. The server trace recorded five Bone Armor casts close together, but the input/packet timing was not captured, so the cause is not established. Completed Bone Armor probes were removed to reduce debugger overhead. Two non-suspending probes now log the client Ascendancy key cooldown decision and the incoming cooldown reply. Next: with cooldown ready, press once, wait five seconds, and report whether activation occurred; do not spam the key during this controlled check.

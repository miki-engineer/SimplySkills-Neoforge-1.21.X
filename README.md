...
- Cooldown fix validation: `gradlew.bat build` passed with Java recompilation; no automated tests exist. After restarting the client, a successful Bone Armor cast confirmed haste 1.0 and an outgoing cooldown of exactly 70000 ms. The user reported a brief freeze and saw 66 afterward; the freeze cause and client display timing remain unconfirmed. Completed cast/cooldown probes were removed to reduce debugger overhead. A visual recheck without those probes and a bonus-haste check remain, followed by the remaining Bone Armor checks. The HUD rounds down whole seconds, so a valid 70-second cooldown may first display 69 after time has elapsed.

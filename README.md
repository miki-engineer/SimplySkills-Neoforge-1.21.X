...
- Current local setup: Arcane Slash selected with **1 spent Ascendancy point**, confirmed by debugger; the home computer's 30-point allocation did not transfer. Initial effect duration is 20 ticks and Arcane Attunement is absent at this tier. Repeat rolls below 80 recast, while observed rolls 82/90/89 stop. User reported a sideways slash, missing character animation, and repeated IDE-console exceptions. Both projectile variants now use `TOWARDS_MOTION` instead of `ALONG_MOTION`, removing the extra 90-degree model rotation in the installed renderer. Both projectile JSON files parsed and the local Gradle build passed. Restart and visually verify the base slash before testing the 30-point variant; damage/piercing and upgraded behavior remain pending. Home-only configs and unlocks must not be assumed here. Signature and Ascendancy use separate keys.

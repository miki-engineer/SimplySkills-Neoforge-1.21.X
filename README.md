...
- Cooldown fix validation: `gradlew.bat build` passed with Java recompilation; no automated tests exist. In-game recheck is pending after restarting the development client with the rebuilt classes. Recheck Bone Armor at normal haste and with bonus haste, then continue the remaining Bone Armor checks. The HUD rounds down whole seconds, so a valid 70-second cooldown may first display 69 after time has elapsed.

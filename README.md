...
User reports all normal animations look good. Reuse this visual confirmation and previous gameplay evidence; optional Prominence, Torment no-target behavior and multiplayer visibility remain unverified. User reported no visible activation for Agony with no target; exact retry cooldown was not measured. Consecration: user observed approximately 58 seconds after restart; source and local config are 60 seconds. Exact initial cooldown/haste was not captured, so the two-second difference is not precisely verified. Next quick regression: toggle Agony off and Torment on, then cast with no nearby enemy. Continue the registered-effect coverage audit without repeating shared mechanics already verified. Buff particle removal still needs a restart check.

...
- Removed eight Prominence-only effects: Focus, Titans Grip and six Melodies. Two-player interactions remain unverified and deferred until a suitable setup or external tester is available.
...
Validation: all spell JSON parses, orientation-only diffs checked, IntelliJ build and Java 21 Gradle build passed. The user confirmed the UI remains normal and Arcane Bolt looks correct after the orientation fix in the external modpack. The tested Bolt variant and flight angles were not specified, so this does not separately verify all four variants. The user caught only a tentative shield glimpse; slowing the game yielded no usable observation. Shield orientation remains unverified. Directional arrow/dagger, shield, meteor/comet and orb representative visual retests, including angled flight, are deferred at the user's request. Earlier visual checks do not establish results for these edits.
...
See [release gates and lower-priority work](release-review.md#release-gates-and-lower-priority-work) for the release checklist, and the explicit limits in each entry above. Packaged dedicated-server startup, second-player interactions, normal progression after bypass removal, specific post-Prominence-removal smoke checks and representative projectile visual retests remain unverified. This documentation reorganization adds no runtime evidence.

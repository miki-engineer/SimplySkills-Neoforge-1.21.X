...
See [release gates and lower-priority work](release-review.md#release-gates-and-lower-priority-work) for the release checklist, and the explicit limits in each entry above. Second-player interactions and packaged dedicated-server startup have not been run. Progression boundary checks, post-removal ability/item smoke checks and focused curse isolation passed; projectile visual smoke checks are user-confirmed. Lower-priority cleanup remains separate from completed gameplay verification.

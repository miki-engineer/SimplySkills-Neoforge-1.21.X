...
- Read README.md and docs/handoff.md before continuing; the latter is a local-only document recording port progress and the current testing handoff. If absent on another computer, request the saved handoff.
- After each completed project change, update docs/handoff.md with the change, validation, and remaining work, then commit and push tracked project changes to origin. Keep docs/ local and ignored; do not add it to Git. Update the public README.md only when player-facing information changes. The user has explicitly requested this workflow; no additional push confirmation is needed.
